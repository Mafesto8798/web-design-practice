...
type ChordProgressionProps = {
    ...
    saveProgression: (progression:Chord[],name:string,key:MusicalKey,mode:Mode) => void;

} 

export const ChordProgression = ({progression,clearProgression,saveProgression,progressionName,updateProgressionName,selectedKey,selectedMode} : ChordProgressionProps) => {
    ...
    return(
        ...
        <div className="text-slate-700 dark:text-slate-200 flex flex-col items-center gap-4 w-full justify-evenly p-2 my-12">
            ...
            <input type="text" placeholder="Progression Name" className="w-5/8 lg:w-1/5 p-2 border-2 border-slate-800 rounded-lg bg-slate-200 text-slate-800 my-12" value={progressionName} onChange={(e) => updateProgressionName?.(e.target.value)} />
            ...
            <span className="flex items-center justify-around w-3/4 lg:w-1/8 my-6">
                ...
                <button className="mx-2 my-2 px-6 py-3  border-4 border-green-700 bg-slate-800 text-slate-200 text-xl  font-semibold active:text-green-400 rounded-lg transition-all duration-100 hover:scale-110 active:scale-85 lg:active:scale-95 hover:cursor-pointer" onClick={() => saveProgression(progression,progressionName,selectedKey,selectedMode)}>Save</button>
            </span>
        </div>
        </>
    )
